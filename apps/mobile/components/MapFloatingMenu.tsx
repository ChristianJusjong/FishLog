...
interface MenuItem {
  ...
  label: string;
  action: 'filter' | 'navigate' | 'save-favorite';
  route?: string;
  color: string;
}
...
export default function MapFloatingMenu({ onFilterPress, showFilter, onSaveFavorite }: MapFloatingMenuProps) {
  ...
  const handleMenuItemPress = (item: MenuItem) => {
    ...
    } else if (item.action === 'save-favorite' && onSaveFavorite) {
      onSaveFavorite();
    }
    toggleMenu();
  };
    ...
    <>
      ...
      <View style={[styles.container, { bottom: insets.bottom + 20 }]}>
        ...
        {menuItems.map((item, index) => {
          const bottom = 138 + (index * 70); // Start 138px up (68 for location button + 70 spacing)
          const isFilterActive = item.action === 'filter' && showFilter;
            ...
            >
              <TouchableOpacity
                style={[
                  ...
                  isFilterActive && styles.activeMenuItem,
                ]}
