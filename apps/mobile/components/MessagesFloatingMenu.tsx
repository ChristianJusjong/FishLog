...
interface MessagesFloatingMenuProps {
  onNewMessage: () => void;
}

const menuItems: MenuItem[] = [
  { icon: 'create-outline', label: 'Ny Besked', action: 'new-message', color: COLORS.accent },
];

export default function MessagesFloatingMenu({ onNewMessage }: MessagesFloatingMenuProps) {
    ...
    <>
      ...
      <View style={[styles.container, { bottom: insets.bottom + 20 }]}>
        ...
        {menuItems.map((item, index) => {
          const bottom = 138 + (index * 70); // Start 138px up (68 for button + 70 spacing)
